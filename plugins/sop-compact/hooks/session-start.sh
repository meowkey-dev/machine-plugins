...
if [[ -z "$LATEST" ]]; then
  ...
  [[ -n "$LATEST" ]] && LEGACY=" (legacy compact-sop snapshot — the PreCompact hook will auto-bootstrap a new \`.claude/sop-compact.md\` on the next compact)"
fi
...
if [[ -n "$LATEST" ]]; then
  ...
  POINTER="You were just compacted (SessionStart source=compact), but no pre-compact handoff was found under \`${REPO_DIR}/.claude/sop-compact/\`. The PreCompact hook auto-bootstraps \`.claude/sop-compact.md\` on first compact, so if this is the very first compact in this repo a bootstrap-*.error.log under \`.claude/sop-compact/\` will show why the bootstrap was skipped. Treat the history above as a lossy summary: re-check live state (git status, gh, files) before acting. Do not start new work until you have re-oriented."
fi
