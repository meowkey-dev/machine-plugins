...
# --- Per-repo opt-out (machine#153). -----------------------------------------------
# Two equivalent signals, either one suppresses the entire sidecar pipeline:
#   1. env `SOP_COMPACT_DISABLED=1` — per-shell escape hatch (cheapest, no stdin read)
#   2. marker file `.claude/sop-compact/disabled` — discoverable, version-controllable
# When either is present we exit 0 silently BEFORE bootstrap, BEFORE snapshot, BEFORE
# any `mkdir -p .claude/sop-compact/` — so a disabled repo never even acquires the
# plugin's on-disk footprint. SessionStart on source=compact then falls through to its
# "no handoff found" pointer (same path as a fresh, never-compacted repo).
if [[ "${SOP_COMPACT_DISABLED:-}" == "1" ]]; then
  exit 0
fi
...
# Marker-file opt-out — resolved against the same REPO_DIR the rest of the hook uses,
# so the check stays consistent with where SOP_FILE / SNAP_DIR get written. `-f` (not
# `-e`) matches the documented `touch .claude/sop-compact/disabled` usage: only a
# regular file (or a symlink to one) trips the opt-out, not e.g. a stray directory.
if [[ -f "${REPO_DIR}/.claude/sop-compact/disabled" ]]; then
  exit 0
fi
...
# Source the bootstrap helper. ${CLAUDE_PLUGIN_ROOT} is set by the hook loader; if it
# isn't (e.g. hand-wired without the plugin loader) we fall back to a path relative to
# this script so the helper still resolves.
LIB_DIR="${CLAUDE_PLUGIN_ROOT:-$(dirname "${BASH_SOURCE[0]}")/..}/hooks/lib"
# shellcheck source=lib/generate-sop.sh
source "${LIB_DIR}/generate-sop.sh"
...
# --- No SOP yet: bootstrap one, then fall through to the normal snapshot path. -----
# v0.5.0 collapsed the old `/init-sop-compact` slash command into this hook so the
# first compact in a fresh repo "just works" — no remembered setup step. We run TWO
# sequential `claude -p` sidecars (bootstrap, then snapshot) rather than a single fused
# one: each prompt does one job, time budgets stay independent, and the bootstrap cost
# only lands once per repo.
#
# Failure policy: if bootstrap fails (rc!=0 or empty output), fall back to the legacy
# "stub handoff + exit 0" path. The snapshot sidecar's prompt assumes an SOP exists
# (it instructs the agent to read it), so running it without one would either fail or
# produce a degraded handoff — and a failed snapshot hard-blocks compaction (exit 2).
# The stub path matches the pre-v0.5.0 behavior for missing-SOP repos and never blocks.
if [[ ! -f "$SOP_FILE" ]]; then
  if generate_sop "$REPO_DIR"; then
    : # SOP now exists — fall through to the snapshot sidecar below.
  else
    TMP="$(mktemp "${SNAP_DIR}/.handoff-${TS}.XXXXXX")"
    {
      printf '# Pre-compact handoff (stub — SOP bootstrap failed)\n\n'
      printf '_Generated %s by sop-compact PreCompact hook (trigger: %s)._\n\n' "$TS" "${TRIGGER:-unknown}"
      printf 'This repo has **no `.claude/sop-compact.md`** and the auto-bootstrap sidecar failed '
      printf '(see the `bootstrap-*.error.log` under `.claude/sop-compact/`). Future compactions '
      printf 'will retry the bootstrap; if it keeps failing, hand-author `.claude/sop-compact.md` '
      printf 'from the plugin template.\n\n'
