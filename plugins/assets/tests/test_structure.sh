#!/usr/bin/env bash
# test_structure.sh — structural smoke for the assets plugin.
...
python3 -c "import json,sys; d=json.load(open('${PLUGIN_DIR}/.claude-plugin/plugin.json')); assert d['name']=='assets' and d.get('version'); " \
  && ok "plugin.json valid (name=assets, has version)" || bad "plugin.json"
