...
### Harness-emitted lifecycle signals (claude-family)

For claude-family launchers (the `*laude*` family — `claude`, `rlaude`, etc.) the plugin injects
CC hooks at dispatch time so the asset's own harness emits lifecycle events deterministically —
no prompt mutation, no pane-pixel heuristics. The signal stream lives at:

```
<signals>/<name>.jsonl       # boot / turn_end / exit events, one JSONL line each
<signals>/<name>.activity    # bare-mtime file; touched on every tool call (PostToolUse)
<signals>/.settings/<name>.json   # generated hook settings; persists for CC's lifetime
```

Example stream:

```jsonl
{"ts":"2026-06-10T04:00:00Z","asset":"pr-151","event":"boot","session_id":"…","cwd":"…","source":"startup"}
{"ts":"2026-06-10T04:07:23Z","asset":"pr-151","event":"turn_end","last_message":"PR opened: https://…"}
{"ts":"2026-06-10T04:07:25Z","asset":"pr-151","event":"exit","reason":"prompt_input_exit"}
```

The `turn_end` event includes a 500-char excerpt of the asset's actual final assistant message
(from the `Stop` hook's `last_assistant_message` field, with a JSONL-transcript fallback).
Replaces the prompted `<signals>/<name>.done` shim and the pane-diff heartbeat for claude
launchers; codex/others keep the existing pane-heuristic path.

Knob (default-on; turn off with either):

```yaml
monitoring:
  harness_signals: false      # disable hook injection for claude-family launchers
```

```
ASSETS_HARNESS_SIGNALS=false  # env override
```

Independent of `--safe` (hooks are observability, not permissions).
...
| `monitoring.heartbeat_interval_min` | `30` | — |
| `monitoring.harness_signals` | `true` | `ASSETS_HARNESS_SIGNALS` |
